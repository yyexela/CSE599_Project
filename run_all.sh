...
#   MNIST  - full shuffle
python -u project.py --epochs=10 --restarts=30 --new_labels=shuffle_full --dataset=MNIST --top_dir=shuffle_full_mnist_mlp_2 --device=cuda:1 > shuffle_full_mnist_mlp_2.txt 2>&1 &
...
python -u project.py --epochs=10 --restarts=30 --new_labels=shuffle_full --dataset=CIFAR10 --top_dir=shuffle_full_cifar10_mlp_2 --device=cuda:2 --learning_rate=0.00005 > shuffle_full_cifar10_mlp_2.txt 2>&1 &
